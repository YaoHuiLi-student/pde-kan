...
def test(model, path, dataset):
    ...
    for i in range(num1):
        ...
        p = model(image)  # forward

        res = F.upsample(p, size=gt.shape, mode='bilinear',
...
def train(train_loader, model, optimizer, epoch, test_path, model_name='PVT-GCASCADE'):
    ...
    if (epoch + 1) % 1 == 0:
        ...
        if DB_dice > ClinicDB_dice:
            print("#############Save ClinicDB_dice#############")
            ClinicDB_dice = DB_dice
            DB_dice = str(DB_dice)
            epoch_db = str(epoch)
            torch.save(model.state_dict(), save_path + '' + epoch_db + '_epoch' + DB_dice + '-best.pth')


if __name__ == '__main__':
    dict_plot = {'CVC-ClinicDB': [], 'Kvasir': [], 'CVC-ColonDB': [], 'CVC-300': [], 'test': []}
    name = ['CVC-ClinicDB', 'Kvasir', 'CVC-ColonDB', 'test']
    ##################model_name#############################
    ...
    parser.add_argument('--train_path', type=str,
                        default='/home/ubuntu/G-CASCADE-main/data/polyp/TrainDataset/TrainDataset/',
                        help='path to train dataset')

    parser.add_argument('--test_path', type=str,
                        default='/home/ubuntu/G-CASCADE-main/data/polyp/TestDataset/TestDataset/',
    ...
    if opt.encoder == 'PVT':
        model = PVT_GCASCADE(n_class=1, img_size=opt.img_size, k=11, padding=5, conv='mr',
                             skip_aggregation=opt.skip_aggregation)
    elif opt.encoder == 'MERIT':
        model = MERIT_GCASCADE(n_class=1, img_size_s1=(256, 256), img_size_s2=(224, 224), k=11,
                               padding=5, conv='mr', skip_aggregation=opt.skip_aggregation)
    else:
